...
export class SelfServeApproval extends BaseProfile {
    ...
    async submitAccessRequest({client, body, logger, ack}) {
        ...
        try {
            ...
            await this.submitRequest(body, client, selectedOption, selectedTime, tgUser, body.user.id);
    ...
    async submitRequest(body, client, selectedGroup, selectedTime, tgUser, slackUserId) {
    ...
    async scheduledMessageTrigger({event, client, body, context, ack, logger}) {
        ...
        // approver message
        let messageString = `Your Full Tunnel Access has expired.\nDuration: ${request.selectedTime} (Start from the time of approval)`
        let msgOption = {
